...
object SparkSQLSample {
   ...
   //1:
    ...
    assmt1.rdd.map(row => s"${row(0)}  ${row(1)}").saveAsTextFile("SparkProjectSQLmaven/Problem1")
    ...
    assmt2.rdd.map(row => s"${row(0)}  ${row(1)}").saveAsTextFile("SparkProjectSQLmaven/Problem2")
    ...
    assmt3.rdd.map(row => s"${row(0)}  ${row(1)}").saveAsTextFile("SparkProjectSQLmaven/Problem3")
    ...
    assmt4.rdd.map(row => s"${row(0)}  ${row(1)}").saveAsTextFile("SparkProjectSQLmaven/Problem4")
    ...
    assmt5.rdd.repartition(1).map(row => s"${row(0)} - ${row(1)}").saveAsTextFile("SparkProjectSQLmaven/Problem5") // keyword repartition(1) prevents building multiple files
